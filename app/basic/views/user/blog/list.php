		...
		<div class="col-lg-8">
			<div class="row">
					...
					<div class="blog-row">
						...
						<span><?php if ($row['is_focus'] == 100): ?>&nbsp;&nbsp;&nbsp;<span title="焦点图" class="glyphicon glyphicon-picture"></span>&nbsp;焦点图<?php endif; ?></span>
							</div>
							<div class="col-lg-4 text-right">
							<a href="/user/blog/edit/<?= $row['id'] ?>"><span class="glyphicon glyphicon-pencil"></span>修改</a>
							<?php if (false && $row['image'] && $row['status'] == 'publish' && $row['is_private'] != 1 && $row['is_focus'] != 1): ?>
						...
						<div class="description <?php if($row['image']): echo 'des-has-image';endif; ?>">
							<?php if($row['image']): ?>
							<div class="image"><a href="<?=$row['image']?>" class="flyout"><img title="" src="<?= $row['image'] ?>"></a></div>
							<?php endif; ?>
							<?= $row['description'] ?>...
						</div>
					</div>
