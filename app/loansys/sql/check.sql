...
create table if not exists `sys_check`(
	...
	`car_brand` int unsigned default 0 comment '车品牌id',
	`car_type` varchar(20) default '' comment '车型号',
	...
	`car_fall_price` float(7,2) default 0.00 comment '每月掉价幅度',
	`car_assess_price` float(6,2) default 0.00 comment '评估价格',
